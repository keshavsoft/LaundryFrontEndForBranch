...
function jFLocalOrderDetails(value, row, index) {
    if (row.IsItems) {
        return row.AggValues.ItemDetails
    }
    else {
        return [
            `<a href="/Laundry/Branch/NewOrders/HtmlFiles/AddItems.html?OrderNumber=${row.pk}" data-ordernumber="${row.pk}" class="btn btn-outline-success SettlementButtonClass" >`,
